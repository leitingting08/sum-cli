        ...
        <sum-layout :settings="settings">
            <div class="sum-breadcrumb">
                <el-breadcrumb separator=">">
                    <el-breadcrumb-item v-for="item in breadcrumbList" :to="{ path: item.path }" :key="item.name">{{
                        item.name
                    }}</el-breadcrumb-item>
                </el-breadcrumb>
            </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const updateBread = (arr, routeValue) => {
            arr.forEach(item => {
                if (routeValue.includes(item.path)) {
                    data.breadcrumbList.push({ name: item.title, path: item.path })
                }
                if (item.children) {
        ...
        watch(
            ...
            newValue => {
                data.breadcrumbList = []
                newValue && newValue !== '/' && updateBread(data.settings.menuData, newValue)
            },
            { immediate: true }
        )
